...
const SocialLinks = () => {
  const links = [
  {
    ...
    href : 'https://www.linkedin.com/in/jay-phan-350285231',
    style: 'rounded-tr-md'
  },
  {
    id: 2,
    child: (<>GitHub  <FaGithub size = {30} /> </>),
    href : 'https://github.com/jayphan14'
  },
  {
    id: 3,
    child: (<>Mail  <HiOutlineMail size = {30} /> </>),
    href : 'mailto:jbphan@uwaterloo.ca'
  },
  {
    id: 4,
      ...
      <ul>
        {links.map((link) => (
          <li key = {link.id} className={'flex justify-between items-center w-40 h-14 px-4 bg-gray-500 ml-[-100px] hover:ml-[-10px] hover:rounded-md duration-300 ' + link.style}>
            <a href={link.href} 
              className='flex justify-between items-center w-40 h-14 text-white'
